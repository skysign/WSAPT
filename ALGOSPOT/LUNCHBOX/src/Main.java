...
/**
 * LUNCHBOX Microwaving Lunch Boxes / ALGOSPOT
 * 문제링크 : https://algospot.com/judge/problem/read/LUNCHBOX
 * 제출링크 : https://algospot.com/judge/submission/detail/657174
 */
public class Main {
    int N;
    int[][] dp;
    ...
    public int lunchBoxes() {
        ...
        for(int i=0; i<N; ++i) {
            Pairs[i] = new MyPair<Integer, Integer>(Es[i], i);
        }
        Arrays.sort(Pairs);
        ...
        int Msum = 0;
        for(int i=0; i<N; ++i) {
            MyPair pair = Pairs[N -i -1];
            Msum += Ms[(int)pair.getValue()];
            r = Math.max(r, Msum + (int)pair.getKey());
        }
