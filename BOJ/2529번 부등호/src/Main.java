 ...
 * 제출링크 : https://www.acmicpc.net/source/19913633
 * 문제풀이 : https://skysign.tistory.com/237
 */
...
public class Main {
    ...
    boolean bt(int idx, int prev) {
        int tried = (bMax)? 10: -1;
        int t = assignNumber(tried);

        if(idx == inequality.length())
            return true;

        while(t >= 0) {
            boolean b = (inequality.charAt(idx) == '<')? prev < t: prev > t;
            if(b) {
                if (bt(idx + 1, t)) {
                    r[idx] = t;
                    return true;
    ...
    int assignNumber(int tried) {
        if(bMax)
            return assignMaxNumber(tried);

        return assignMinNumber(tried);
    }

    int assignMaxNumber(int tried) {
        for(int i=candidate.length-1; i>=0; --i) {
            if(candidate[i] == 1 && i < tried) {
                candidate[i] = 0;
                return i;
            }
        }
