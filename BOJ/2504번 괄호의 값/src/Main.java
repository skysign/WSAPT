 ...
 * 제출링크 : https://www.acmicpc.net/source/17881672
 */
public class Main {
    final int TYPE_C = 0;
    final int TYPE_V = 1;
    ...
    public void solve() throws IOException {
        ...
        ArrayList<Pair<Integer,Integer>> ral = new ArrayList();
    ...
    public boolean IsMatched(ArrayList<Pair<Integer,Integer>> ral, char right) {
        boolean bV = true;
        int sum = 0;
        ...
        while(bV) {
            ...
            pair = ral.get(ral.size()-1);
            ral.remove(ral.size()-1);

            if(TYPE_V == (int)pair.getKey()) {
                sum += (int)pair.getValue();
            }
            else {
                bV = false;
        ...
        if(')' == right) {
            if('(' == (char)pair.getValue()) {
                sum = (sum == 0)? 2: (sum * 2);
                ral.add(new Pair(TYPE_V, sum));
                return true;
            }
