...
/**
 * 고대어 사전 / ALGOSPOT / V2 풀이 O(N)
 * 문제링크 : https://algospot.com/judge/problem/read/DICTIONARY
 * 제출링크 : https://algospot.com/judge/submission/detail/664902
 * 문제해설 : https://skysign.tistory.com/197
 */
public class Main {
    int N;
    int[][] map;
    ...
    public void makeGraph(ArrayList<String> al) throws IOException {
        ...
        for(int i=0; i<al.size()-1; ++i) {
            ...
            for(int l=0; l<len; ++l) {
                ...
                if(iFm != iTo) {
//                    map[iFm][iTo] = 1;
                    Vertex fm = hsmap.get(iFm);
                    if(null == fm) {
                        fm = new Vertex(iFm);
                        hsmap.put(iFm, fm);
                    }
                    Vertex to = hsmap.get(iTo);
                    if(null == to) {
                        to = new Vertex(iTo);
                        hsmap.put(iTo, to);
                    }
                    DirectedEdge edge = new DirectedEdge(fm, to);
                    fm.mal.add(edge);
                    break;
                }
            }
    ...
    public ArrayList<Integer> topologicalSort() {
        ...
        Set<Integer> keys = hsmap.keySet();
        for(Integer key: keys) {
            int idxFm = key.intValue();
            if(!mbVisited[idxFm]) {
                if(DFS(hsmap, idxFm, al) < 0) {
                    return null;
                }
            }
        }
...
//        }
    ...
    public int DFS(HashMap<Integer, Vertex> hsmap, int idxFm, ArrayList<Integer> al) {
        mbVisited[idxFm] = true;

        Vertex fm = hsmap.get(idxFm);

        for(DirectedEdge edge: fm.mal) {
            int idxTo = edge.mTo.mV;
            if(!mbVisited[idxTo]) {
                if(DFS(hsmap, idxTo, al) < 0){
                    return -1;
                }
            }
