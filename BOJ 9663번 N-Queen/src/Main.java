 ...
 * 제출링크 : https://www.acmicpc.net/source/19653593
 * 문제풀이 : https://skysign.tistory.com/216
 */
...
public class Main {
    ...
    public void solve() throws IOException {
        ...
        int r = backTracking2(0);

        // 모든 경우의 수 back tracking을 하는 코드
//        int[][] visited = new int[N][N];
//        int[][] map = new int[N][N];
//        int r = backTracking(map, visited, 1);

        println(r);
    }

    boolean validate(int row) {
        for(int r = 0; r < row; ++r) {
            // queen이 같은 column 에 있는지 확인
            if(queens[r] == queens[row])
                return false;
            // queen이 대각선 위치에 있는지 확인
            if(Math.abs(queens[r] - queens[row]) == Math.abs(r - row))
                return false;
        }
    ...
    public int backTracking2(int row) {
        int r = 0;
...
//    모든 경우의 수 back tracking을 하는 코드
//    N(10)만 되도, 너무 느려서, 수초내에 답이 계산되지 않습니다.
//    private int backTracking(int[][] map, int[][] visited, int depth) {
//        int r = 0;
//
//        for(int i=0; i<N; ++i) {
//            for(int j=0; j<N; ++j) {
//                if(IsEmpty(i, j, map, visited)) {
//                    if(depth == N) {
//                        r++;
//                    }
//                    else {
//                        int clonedMap[][] = clone2D(map);
//                        visit(i, j, clonedMap, visited, depth);
//                        int clonedVisited[][] = clone2D(visited);
//                        r += backTracking(clonedMap, clonedVisited, depth+1);
//                    }
//                }
//            }
//        }
...
//    }
//
//    private boolean IsEmpty(int i, int j, int[][] map, int[][] visited) {
//        if((0 == map[i][j]) && (0 == visited[i][j])) {
//            return true;
//        }
//
//        return false;
//    }
//
//    public void visit(int si, int sj,
//                      int[][] map, int[][] visited, int depth) {
//        visited[si][sj] = depth;
//        map[si][sj] = depth;
//
...
//        }
    ...
    public class _Pair<K,V> {
        ...
        public K L() { return key; }
        private V value;
        public V getValue() { return value; }
        public V R() { return value; }
