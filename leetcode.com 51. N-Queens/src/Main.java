 ...
 * 제출링크 : https://leetcode.com/problems/n-queens/submissions/
 */
public class Main {
    int N;
    int[][] map;
    ...
    ArrayList<Integer> alJ;

    public void solve() throws IOException {
        N = sc.nextInt();

        List<List<String>> r = solveNQueens(N);
    ...
    public void sq(int begI, int begJ, int idx, int N) {
    ...
    public void gen() {
        ...
        for(int k = 0; k<alI.size(); ++k) {
            int i = alI.get(k);
            int j = alJ.get(k);
            char[] s = rtn.get(i).toCharArray();
            s[j] = 'Q';
            rtn.set(i, new String(s));
        }
    ...
    void unmark(int i, int j, int idx) {
        ...
        alJ.remove(alJ.size()-1);
    }

    void _domark(int i, int j, int ori, int idx) {
        for(int k=0; k<d8i.length; ++k) {
            ...
            while(0<= markI && markI < N &&
                    0<= markJ && markJ < N) {
                if(map[markI][markJ] == ori)
                    map[markI][markJ] = idx;
