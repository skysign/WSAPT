...
/**
 * NERD2 너드인가, 너드가 아닌가? 2 / ALGOSPOT
 * 문제링크 : https://algospot.com/judge/problem/read/NERD2
 *
 * 전에 풀었던 FORTRESS 이어서 이것도 RTE입니다. 흠..ㅡ.ㅡ;
 * 문제의 예제입력은 통과하고, 제출하면 RTE 오답입니다.
 * ALGOSPOT은 테스트 케이스를 공개하지 않아서, 오답 디버깅은 하지 않겠습니다.
 *
 * ps : 오늘 일진이 좀 사나운가 봅니다. - -;
 */
public class Main {
    int N;
    int[][] dp;
...
//            println(r);
    ...
    public int NERD2(int N, int[] ps, int[] qs) {
        int r = 0;
        /*
         * ps 값으로 key로 사용하고, 2차원의 x로 사용합니다.
         * qs 값으로 value를 사용하고, 2차원의 y로 사용합니다.
         */
        ...
        for(int i=0; i<N; ++i) {
            ...
            if(map.containsKey(key)) {
                ...
                Integer hiKey = map.higherKey(key);
                if(null == hiKey) {
                    map.put(key, value);
                    SortedMap<Integer,Integer> lowerXs = map.headMap(key);
                    for(Map.Entry<Integer,Integer> entry : lowerXs.entrySet()) {
